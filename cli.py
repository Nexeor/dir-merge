...
def prompt_user_options(msg: str, options: List[str]) -> int:
    while True: # Loop until valid input or exit
        try: 
            ...
            user_input = prompt(">>> ", validator=NumberValidator())
            if not (0 < user_input < len(options) + 1):
                raise ValueError(f"Value {user_input} is out of range. Must be between 1 and {len(options)}")
            return user_input
        except (ValueError, Validator) as e:
            print(f"\nError: {e}")
        except EOFError:
